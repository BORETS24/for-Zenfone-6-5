...
/*	ASUS_BSP: Louis, show frame update for debug ++	*/
static int disp_frame_update;
static inline int GetDisplayFrameUpdate(void)
{
	return disp_frame_update;
}

void SetDisplayFrameUpdate(int frames_cnt)
{
	disp_frame_update = frames_cnt;
}
EXPORT_SYMBOL(SetDisplayFrameUpdate);
/*	ASUS_BSP: Louis --	*/
			...
			struct mdfld_plane_contexts *psContexts)
{
	...
	/* ZE500CL001S */
	if ((GetResumeStarted() || GetFakeResume()) && !GetResumeState(EResumeTypeDisplayOn)) {
